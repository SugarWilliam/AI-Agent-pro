...
| **输出形态** | 结论简洁可操作；数据与来源注明；矩阵/情景双重决策；强支撑位与多层次退出；**每份报告必含魔鬼代言人压力测试**；建议提交量化幻方矩阵标准委员会署名评估与审核；质量基准为《量化幻方矩阵投资决策报告【V2.0钻石标准】》；协议栈版本 V3.0 |
...
| 〇·3 | 数据贞操与溯源协议（V3.0） | 关键数据精确来源；报告末尾**数据溯源表**；禁止模糊表述；存疑数据剔除或敏感性分析 | rag_quant_output_protocols |
| 〇·4 | 立体交叉验证框架（V3.0） | 标的置于「行业景气度(X)-公司相对优势(Y)-市场系统性风险(Z)」三维坐标系；同业含动态估值对比与溢价/折价历史分位 | rag_quant_output_protocols |
| 〇·5 | 专业级钻石标准增强协议（V3.0） | 逻辑量化锚定（每评分/权重+条件调整公式）、魔鬼代言人+反驳所需证据、历史估值分位、风险警示与个性化揭示、禁止动作条款、数据加工过程标注、情景冲击力、概率-结论量化、KDI 融合与调整公式、PE-Band/DCF/回测/组合/可比细化/宏观政策/技术面/用户个性化、标准委员会签署（认证结果、归档编号、有效期限）；**必须体现**现金流分析专节、3×3 估值矩阵（估值类不可省略）、三维坐标显式小标题、时间止损条款、动态情绪因子 | rag_quant_output_protocols |
...
| 六·2 | 逻辑量化锚定协议（V3.0） | 主观判断须有客观量化依据或动态调整公式；鼓励 KDI 触发调整公式；输出时注明权重与概率依据 | rag_quant_output_protocols |
| 七 | 情景动态概率与双重决策体系 | 初始概率、KDI 权重、多时间维度沙盘、量化矩阵+情景概率、按季度更新 | skill_scenario_dynamic_probability, skill_multi_temporal_sandbox |
| 八 | 强支撑位与多层次退出 | **PE Band 下轨**；**短期支撑位**结合成交量、均线等技术面；多层次退出；**八·1 程序化操作清单（V3.0）**：阶梯式交易执行手册、KDI 监控与自动警报清单；**重大决策与行为护栏（可选）** | skill_behavior_guardrails；rag_quant_output_protocols；预验尸/魔鬼代言人/行为约束见绑定 skill 与 RAG「决策与行为协议」 |
...
| - | 【输出规范】 | **V3.0 强制 8 段**：须含**现金流分析**、**3×3 估值矩阵**（估值类不可省略）、**三维坐标显式小标题**、KDI 含**动态情绪因子**、执行手册含**时间止损**；可融入**期望值**表述与**霍华德·马克斯式**周期/风险/安全边际（与锚定结合）；**魔鬼代言人必含**+反驳所需证据；**标准委员会签署**；禁止动作、个性化风险揭示、概率-结论量化 | 以《量化幻方矩阵投资决策报告【V2.0钻石标准】》为质量基准；须达 V3.0 专业级顶级水准 |
...
| skill_devil_advocate | AI魔鬼代言人 | 对报告与结论做对立面反驳、逻辑漏洞与未证实假设检查 | **每份报告必须**包含魔鬼代言人环节（V3.0 强制） |
...
| rag_quant_output_protocols | 量化幻方报告输出协议 | 数据贞操与分级响应、强观点、决策路径图、同业比较与五条技术条款、王者级四要件；**V3.0**：数据溯源表、立体交叉验证、逻辑量化锚定、阶梯式执行手册、KDI 警报清单、8 段结构、魔鬼代言人必含、标准委员会署名审核；专业级增强（22 项）：逻辑量化锚定与条件调整公式、魔鬼代言人+反驳所需证据、历史估值分位、风险警示与个性化揭示、禁止动作条款、数据加工过程、情景冲击力、概率-结论量化、KDI 融合与调整公式、PE-Band/DCF/回测/组合/可比细化/宏观政策/技术面/用户个性化、标准委员会签署（认证结果、归档编号、有效期限）；**动态适应**：协议中百分比/阈值/代码格式为规范模板，具体数值与标的本次会话动态决定。 |
...
- **必须**：数据甄别/分层/事实与观点/逻辑与时效、量化矩阵与情景概率双重决策、强支撑位与多层次退出、数据来源与时间与本报告时间、**先 RAG/网络检索再输出（禁止占位与模拟）**、CN 框架结合（凡涉及 A 股）；**V3.0**：**每份报告必须包含魔鬼代言人环节**；报告末尾须含标准委员会签署（认证结果、归档编号、有效期限）；报告结构须符合 V3.0 强制 8 段（含数据溯源表、3×3 估值压力测试、阶梯式交易执行手册、报告使用指南与版本更新日志）。
...
- **V3.0 动态适应**：`rag_quant_output_protocols` 开篇已增**动态适应声明**：协议中的百分比、阈值、代码格式等为规范模板或方法论示例，具体数值、标的、时间节点均由本次会话的用户输入与检索结果动态决定，禁止照搬协议示例为报告结论或数据。
...
- **边界与冲突**：各协议块（〇·1～〇·5、一～十三、输出规范）职责清晰，无互斥约束；数据层/矩阵/情景/KDI/魔鬼代言人/行动清单/委员会签署各司其职，未发现协议冲突。
...
- **覆盖度**：钻石标准报告所具备的要素（核心结论概率分布、数据层与溯源表、三维定位、同业动态仪表盘、量化矩阵含依据、决策路径图 Mermaid、KDI 健康线/警报线/自动警报观察项、魔鬼代言人质疑、3×3 估值压力测试、阶梯式执行手册含纪律、报告使用指南与版本更新日志与免责、末尾签署）均在 V3.0 协议与输出规范中有**强制或强烈建议**对应；且 V3.0 在逻辑量化锚定、魔鬼代言人+反驳所需证据、历史估值分位、个性化风险揭示、禁止动作、数据加工过程、情景冲击力、概率-结论量化、KDI 融合与调整公式、PE-Band/DCF/回测/组合/可比细化/宏观政策/技术面/用户个性化、标准委员会签署（认证结果、归档编号、有效期限）等方面**超出**钻石标准样本。
- **结论**：从协议与能力覆盖上，**已具备**稳定达到或超越《量化幻方矩阵投资决策报告【V2.0钻石标准】》水平的结构性条件；实际报告质量还依赖模型遵循度与当次数据可得性，建议通过持续生成与抽样评审做闭环验证。
...
- **版本**：本设计文档与 v8.5.3 的量化幻方助手实现一致；后续若增删 skill/RAG 或调整 systemPrompt 结构，应同步更新本文档。
