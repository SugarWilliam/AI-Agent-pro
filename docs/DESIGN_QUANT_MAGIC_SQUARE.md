...
| 〇·5 | 专业级钻石标准增强协议（V3.0） | 逻辑量化锚定（每评分/权重+条件调整公式）、魔鬼代言人+反驳所需证据、历史估值分位、风险警示与个性化揭示、禁止动作条款、数据加工过程标注、情景冲击力、概率-结论量化、KDI 融合与调整公式、PE-Band/DCF/回测/组合/可比细化/宏观政策/技术面/用户个性化、标准委员会签署（认证结果、归档编号、有效期限） | rag_quant_output_protocols |
...
| - | 【输出规范】 | **V3.0 强制 8 段**（同上）；**魔鬼代言人必含**且每项质疑须给出**反驳所需证据**；**标准委员会签署**（认证与归档、认证结果、归档编号 QHFM-Archive-代码-日期、有效期限、署名及免责）；禁止动作条款、个性化风险揭示、概率-结论量化；结论清晰、数据来源与时间、可读性、合规提示 | 以《量化幻方矩阵投资决策报告【V2.0钻石标准】》为质量基准；须达 V3.0 专业级顶级水准（22 项增强） |
...
- **版本**：本设计文档与 v8.5.2 的量化幻方助手实现一致；后续若增删 skill/RAG 或调整 systemPrompt 结构，应同步更新本文档。
