...
- ✅ **跨平台**：支持任何静态托管服务
- ✅ **PWA支持**：可安装为桌面/移动应用（含 manifest、图标、Service Worker）
- ✅ **离线使用**：支持离线模式（Service Worker 缓存，需先在线加载一次）
...
项目支持安装为桌面/移动应用，安装后有独立图标和启动方式：

| 平台 | 安装方式 |
|------|----------|
| **PC (Chrome/Edge)** | 地址栏右侧「安装」图标，或菜单 → 安装 AI Agent Pro |
| **Android** | Chrome 菜单 → 添加到主屏幕，或浏览器安装提示 |
| **iOS** | Safari 分享 → 添加到主屏幕 |

**要求**：需通过 HTTPS 或 localhost 访问（Service Worker 仅在安全上下文下注册）。
...
- 配置密钥后，可完整解析文档内容
- **CORS**：浏览器直连 `r.jina.ai` 可能被拦截；GitHub Pages 部署通常无此问题；本地开发若遇 CORS 错误，可尝试使用 develop 分支的 `server.py`（含 Jina 代理）
