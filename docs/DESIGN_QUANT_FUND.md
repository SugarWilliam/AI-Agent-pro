...
| **L1** | M01 数据融合 | 数据分级、补全清单、溯源 | `skill_data_cleaning`、`skill_analyst`、`rag_data_api_finance`、`rag_data_api_official` |
...
### 2.2 RAG（21 项：1 个 F6 内置摘要 + 20 个扩展库）

| 顺序 | id | 作用 |
|------|-----|------|
| 1 | `rag_quant_fund_f6` | F6.0 协议摘要（alwaysInject） |
| 2 | `rag_cn_analysis_framework` | CN 政策/市场/行为框架 |
| 3 | `rag_decision_behavior_protocols` | 决策与行为护栏 |
| 4 | `rag_value_investment` | 价值投资与组合思想 |
| 5 | `rag_finance` | 通用财经 |
| 6 | `rag_data_api_finance` | 数据 API 说明 |
| 7 | `rag_data_api_official` | 官方数据口径 |
| 8 | `rag_sse` | 上交所规则与披露语境 |
| 9 | `rag_szse` | 深交所规则与披露语境 |
| 10 | `rag_snowball_realtime` | 雪球等实时财经源 |
| 11 | `rag_industry_reports` | 行业报告 |
| 12 | `rag_government_reports` | 政府/政策报告 |
| 13 | `rag_social` | 社会结构等 |
| 14 | `rag_first_principles` | 第一性原理 |
| 15 | `rag_logic` | 逻辑 |
| 16 | `rag_iceberg_model` | 冰山模型 |
| 17 | `rag_psychology` | 心理学 |
| 18 | `rag_neuroscience` | 脑科学（与认知补充） |
| 19 | `rag_temporal_logic` | 时序逻辑 |
| 20 | `rag_common_sense` | 常识 |
| 21 | `rag_history` | 历史类比 |
...
用户消息（当前助手 = quant_fund）
  ...
  → buildEnhancedSystemPrompt
        ① 角色 + description
        ② llm.js：【当前日期与时间】【约束重申】【署名】【期望值/橡树硬性小节】
...
| 专属 RAG | 含 `rag_quant_output_protocols` | **不含**；含 `rag_quant_fund_f6` |
| 系统提示注入 | 〇·6/〇·7 显式 + 矩阵专用 | 期望值 + 橡树 + 基金数据约束 |
