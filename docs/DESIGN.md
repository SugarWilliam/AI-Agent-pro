...
**定义**：量化幻方矩阵（SubAgent id: `quant_magic_square`）是面向**专业价值投资分析**的专用助手，主要服务**中国股市**（A 股/基金/政策/企业），输出**可复现、可操作**的矩阵评分、情景概率与报告结论。

**核心定位**：主战场（股票、基金、数据分析、政策与企业研究）；方法论（高级量化幻方与决策矩阵、BMP 选股、情景动态概率、CN 本土化分析框架）；输出形态（结论简洁可操作、数据与来源注明、矩阵/情景双重决策、强支撑位与多层次退出）；边界（不替代用户决策、行为护栏为可选、仅供个人学习与研究方法论使用）。

**与其它助手的关系**：超级决策偏通用决策与认知偏差；工作秘书偏项目协调、可调度量化幻方；通用助手无投资专用能力。**导出与下载（全助手通用）**：单条消息下载与对话导出（MD/HTML/PDF）仅包含报告内容、不包含 AI 思考过程；H5 下载为报告内容块。
...
> **v8.5.1 更新**：A2A 自主调度已实现，详见 [DESIGN_A2A.md](DESIGN_A2A.md)。以下保留为历史差距分析参考。
