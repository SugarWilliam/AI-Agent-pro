...
- **V3.0 强制报告结构**：①核心结论 ②数据层与溯源表 ③立体分析与量化矩阵 ④决策路径图 ⑤KDI 与警报清单 ⑥压力测试（魔鬼代言人+3×3 估值压力测试矩阵）⑦阶梯式交易执行手册 ⑧报告使用指南、版本更新日志、免责声明。
- **魔鬼代言人**：每份报告必须包含，对核心结论进行最严厉反驳与压力测试。
- **标准委员会**：报告末尾须含委员会签署：认证与归档、认证结果、归档编号（QHFM-Archive-代码-日期）、有效期限、署名及免责声明。

八、专业级钻石标准增强（V3.0，须达专业级顶级水准）
...
- **操作纪律与禁止动作**：明确禁止动作条款（如严禁抄底补仓直至达到最终状态）；操作纪律与可操作原则清晰。
...
- **模型深度**：PE-Band、DCF 安全边际、情绪指标（隐含波动率、舆情）若适用。
...
- **标准委员会签署**：每份报告须含委员会签署：最终认证与归档、认证结果、归档编号、有效期限、署名及免责。

九、必须体现与思想融合（V3.0 增补）
- **必须体现**：①**现金流分析**（独立小节或明确标题：OCF/FCF/OCF÷净利/Capex/营运资本）；②**3×3 估值压力测试矩阵**（估值类报告不可省略为文字概括）；③**三维坐标定位**（X/Y/Z 显式小标题）；④**时间止损**（执行手册中与价格/逻辑止损并列，含日历或事件节点）。
- **动态情绪因子**：KDI 或情景中纳入可更新情绪指标（IV、融资余额、北向、板块资金、舆情等，以检索为准），注明本轮取值与调整规则。
- **期望值与工程化**：E(结果)=ΣP×结果；范围管理（假设上下界）；结论落到检查清单、触发条件、复查日期。
- **霍华德·马克斯/橡树资本**：第二层次思维、周期与钟摆、风险=永久资本损失、安全边际与逆向（须与数据锚定结合）。`
        }
    ];
...
〇·5、专业级钻石标准增强协议（V3.0，须达专业级顶级水准）
...
- **模型深度扩展**：做 **PE-Band 分析**（当前估值在历史周期中的位置）；可引入 **DCF** 测算内在价值与安全边际；可增加情绪指标（期权隐含波动率、舆情评分）。
...
- **标准委员会签署（每份报告必含）**：报告末尾须由**量化幻方矩阵标准委员会**签署，含：**最终认证与归档**（报告质量、逻辑与结论获认证）；**认证结果**（如「予以通过，准予归档」）；**归档编号**（格式如 QHFM-Archive-股票代码-YYYYMMDD）；**有效期限**（至下一次强制复查节点，如年报/一季报发布日）；到期前如发生重大事件（如业绩预告大幅偏离预期）报告需提前更新；委员会署名及「本审核报告基于量化幻方矩阵方法论生成，旨在提升投资决策的理性与系统性，不构成任何投资建议」类免责声明。
...
七、情景动态概率与双重决策体系
- **情景动态概率模型**：用于动态概率调整。**初始概率**应采用历史相似技术突破/案例的统计结果，或通过**蒙特卡洛模拟**基于关键变量（需求增速、份额、定价等）的分布推导；**KDI（关键驱动指标）权重**应通过**回归分析**历史数据，确定各指标对股价或盈利影响的弹性系数后设定，避免主观赋权；输出时注明权重依据与更新公式
...
- **八·1、程序化操作清单协议（V3.0）**：操作建议必须细化为 **「阶梯式交易执行手册」**，明确价格/条件阶梯、对应仓位比例及同步必须满足的基本面条件；KDI 仪表盘须升级为 **「KDI 监控与自动警报清单」**，指明可设置的日常观察项（如预警价、监控事件、警报线）。
...
【输出规范】报告质量须达到或超过《量化幻方矩阵投资决策报告【V2.0钻石标准】》；规范以 V3.0 全栈协议为准。
1. **报告结构（V3.0 强制 8 段）**：①**核心结论（概率-结论）** 强观点与核心操作，可融入**期望值**表述（各情景概率×结果）；②**数据层与溯源** 须含**现金流分析专节或明确标题**（OCF/FCF/OCF÷净利/Capex 等）及**数据溯源表**（报告末尾须附）；③**立体分析与量化矩阵** 须含**三维坐标定位**（X/Y/Z **显式小标题**+各轴结论）、量化决策矩阵及深度分析；④**决策路径图** 基于 Mermaid 的基本面信号驱动流程图；⑤**KDI 监控与自动警报清单** 须含**动态情绪因子**（本轮取值、来源、更新时间及对概率/仓位的调整规则）；⑥**压力测试** 必含**AI 魔鬼代言人**（含反驳所需证据）及**3×3 估值压力测试矩阵**（个股/基金估值类报告**禁止省略**）；⑦**阶梯式交易执行手册** 须含**价格/条件阶梯、仓位、同步基本面条件**，并**必须**写明**时间止损**条款（日历或事件节点）；⑧**报告元信息** 含**报告使用指南**、**版本更新日志**及免责声明。分析中须体现**霍华德·马克斯式第二层次思维、周期/风险/安全边际**（与数据锚定结合，非空泛引用）。
2. **魔鬼代言人与标准委员会**：每份报告**必须**包含魔鬼代言人环节，对核心结论进行反驳与压力测试，且对每项质疑须给出**「反驳所需证据」**（何种数据/事件可推翻结论）；报告末尾须含**量化幻方矩阵标准委员会签署**：认证与归档说明、认证结果（如予以通过/准予归档）、归档编号（QHFM-Archive-代码-日期）、有效期限至下一次强制复查节点、委员会署名及免责声明。
...
9. 涉及个股与基金时，注明数据来源与时效，并做合规与风险提示。`,
            capabilities: ['事实与观点识别', '逻辑严谨', '时序正确', '时效标注与陈旧信息处理', '数据甄别', '数据分类', '数据分层', '数据清洗', '深度分析', '重要程度识别', '优先级排序', '权重设定', '数据档案事实层解读层推断层', '数据贞操与分级响应ABC', '数据溯源表与溯源协议', '立体交叉验证三维定位', '逻辑量化锚定与KDI触发公式', '阶梯式交易执行手册', 'KDI监控仪表盘', 'KDI监控与自动警报清单', '3x3估值压力测试矩阵', '报告使用指南与版本更新日志', 'AI魔鬼代言人报告鲁棒性测试', '魔鬼代言人必含', '标准委员会署名评估与审核', '标准委员会签署归档与编号', '反驳所需证据', '历史估值分位', '禁止动作条款', '数据加工过程标注', '个性化风险揭示', '概率结论量化表达', 'PEG与ROE-PB可比细化', '宏观政策因子分析', '技术面强化与筹码分布', '用户个性化路径与持仓占比', 'PE-Band与DCF模型深度', 'KDI与矩阵回测验证', '组合视角与相关系数', '强观点概率结论输出', '决策路径图与复查节点', '同业估值可比公司强制', '高频数据具象化', '预期数据三重验证', '技术数据零假设', '时间线动态校准', '王者级报告四要件', 'V3.0钻石标准报告结构', '现金流分析必须体现', '3x3估值矩阵必须体现', '时间止损必须体现', '三维坐标定位必须体现', '动态情绪因子', '期望值思维与工程化落地', '霍华德马克斯橡树资本思想', '高级量化幻方', 'BMP选股框架', '定量财务分析', '现金流与盈利质量深化', '定性竞争优势', '信息数据可靠性识别', '数据高级分析', '估值深度动态PE与PEG', 'PE Band下轨与增强算法', '短期支撑位成交量均线技术面', '行业差异化分析', '行业周期与经济周期四维分析', '高级量化决策矩阵', '情景动态概率模型', '多时间维度沙盘推演', '量化矩阵与情景概率双重决策体系', '强支撑位量化标准', '多层次退出机制', '策略回测', '对冲方案设计', '依赖关系分析', '时序关系分析', 'CN分析框架', '政策β质量α行为γ', '中国特色质量价值', '中国A股多因子', '中国周期与择时', '中国版核心-卫星', '专精特新与小盘策略', '中国全天候', '重大决策预验尸与二阶思维', '行为约束协议', '实时多源数据收集', '可连通时须用RAG数据', '随时调用全部RAG与网络并可向用户索要补充', 'RAG与网络精准全面调用', 'ROE与股东盈余', '毛利率与留存利润效率', '中国市场政策与估值', '结论简洁可操作', '数据充分论证严谨', '报告来源与时间注明'],
            modelPreference: ['deepseek-reasoner', 'glm-4-plus', 'gpt-4o'],
