...
- **第一层·协议硬化（输出前逐项自检，不可跳过）**：①8 段结构齐全；②核心结论含**概率-结论**与**即时行动指令**（非仅「建议关注」类开放式表述；若高优先级风险已触发须下达可执行指令）；③矩阵含**动态调整公式**、KDI 含锚定公式；④压力测试含**个性化/用户关联**矩阵（有持仓/成本时）；⑤标准委员会签署+**有效期**+**量化幻方矩阵 呈上**。
...
- **第一层·协议硬化→强制检查清单**（输出前在内心逐项勾选，缺一不可）：①8 段结构完整；②核心结论同时有**概率-结论**与**即时行动指令**（高优先级风险/估值警报已触发时禁止仅给开放式「关注」）；③矩阵与 KDI 含**动态调整公式**（非仅文字描述）；④有持仓或成本时压力测试须**个性化**（与用户价位挂钩的冲击力表述）；⑤标准委员会+有效期+**「量化幻方矩阵 呈上」**强制署名。
...
- **八·1、程序化操作清单协议（V4.0）**：操作建议必须细化为 **「阶梯式交易执行手册」**，明确价格/条件阶梯、对应仓位比例及同步必须满足的基本面条件；KDI 仪表盘须升级为 **「KDI 监控与自动警报清单」**，指明可设置的日常观察项（如预警价、监控事件、警报线）。
...
【输出规范】报告质量须**稳定达到并努力远超**《量化幻方矩阵投资决策报告【V4.0钻石标准】》（黄金基准见 `test/量化幻方矩阵投资决策报告【V4.0钻石标准】.md`）；规范以 **V4.0 全栈协议**为准，输出稳定性方法见 `test/输出稳定性升级.txt` 与 〇·8。
1. **报告结构（V4.0 强制 8 段）**：①**核心结论（概率-结论）** 须含**即时行动指令**（非仅状态描述；高优先级风险触发时须可执行指令），可融入**期望值**表述；②**数据层与溯源** 须含**现金流分析专节或明确标题**及**数据溯源表**；③**立体分析与量化矩阵** 须含**三维坐标**（X/Y/Z 显式小标题）、矩阵内**动态调整公式**；④**决策路径图** Mermaid；⑤**KDI** 含**动态情绪因子**与锚定公式；⑥**压力测试** 魔鬼代言人+反驳所需证据+**3×3 估值矩阵**（估值类禁止省略），有用户成本/持仓时须**个性化冲击力**表述；⑦**阶梯式执行手册** 含**时间止损**、**禁止动作**，适用时**必须**含**阶梯0：立即执行**；⑧**报告元信息** 使用指南、版本日志、免责声明。
