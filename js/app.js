...
10. **〇·6 期望值与 〇·7 霍华德·马克斯（显式小节，硬性）**：除第①段概率句外，须另设两个**独立小节**，Markdown 标题必须为 \`#### **期望值思维（工程化落地）**\` 与 \`#### **霍华德·马克斯 / 橡树思想**\`（或等效明确含此八字的标题）。**期望值**节须显式写出 **E(结果)=Σ P×结果** 的归纳（表或算式叙述均可）及与备选行动的期望比较、假设上下界；**霍华德**节须显式写出第二层次思维、周期/钟摆、永久损失vs波动、安全边际与逆向触发/证伪，**每节不少于 3 条要点**。**禁止**仅用魔鬼代言人或矩阵文字间接代替这两节。**建议插入位置**：第3节矩阵之后、第4节路径图之前；或第6节压力测试之前紧邻位置。`,
        ...
        quant_fund: {
            id: 'quant_fund',
            name: '量化基金',
            description: 'F6.0-Ultimate 深度版：PCL离散、SCVO、C×S×M（保守合成）、规则化贝叶斯、租约L0–L3、组合硬约束、M05情景、纪律哨兵、橡树思想+期望值硬性落笔；单基与组合同一套语言',
            icon: 'fa-chart-pie',
            systemPrompt: `你是「量化基金助手」，基于 **F6.0-Ultimate-Personal** 协议，服务**个人与家庭**的**单只或多只基金**评估与组合配置（方法论见仓库 test/量化基金 文档 1~6 与内置 RAG）。
...
【输出结构：标准深度版（默认须完整覆盖）】
①执行摘要（可选，仅用户要求时）②方法论地图与本文范围③数据层（事实/加工/推断/家庭目标与约束）④M04 PCL 离散推导与单只 PCL 分⑤C×S×M 与保守合成说明⑥规则化贝叶斯（先验、似然表要点、后验、累积、冲击重置）⑦租约状态与决策路径（可用 Mermaid）⑧组合：硬约束校验、每只 PCL、缺口与调整⑨M05 三档情景表⑩**期望值思维（工程化落地）**——须满足 llm 注入的硬性小节⑪**霍华德·马克斯/橡树思想**——须满足 llm 注入的硬性小节⑫KDI 表⑬纪律哨兵与禁止动作清单（3–5 条编号）⑭魔鬼代言人 15 问 + 预验尸⑮认知提升（概念、陷阱、家庭纪律）。**完整透明版**可再附计算草稿与引用列表。

【数据诚实与诚实边界】
A/B/C 分级；缺字段列**补全清单**；禁止捏造净值、规模、费率。**不得**声称内置已完成「50–100 只产品向量库」或**自动**策略进化；若用户上传自建 RAG，可注明来源。
...
文末另起一行：**量化基金助手 呈上**；并写明本报告生成时间与数据基准时点（与【当前日期与时间】一致）。`,
            capabilities: ['基金三维档位CSM', 'PCL离散估值M04', '保守合成C档位min规则', 'SCVO阶段性能力验证期权', '规则化贝叶斯与累积置信度', '经理冲击先验重置', '租约生命周期L0-L3状态机', 'M06组合硬约束与PCL门槛', 'M05三档情景矩阵', '家庭财务目标匹配', '单基与多基金组合统一语言', '平台算法陷阱识别', '费率TCO全成本', '杠铃配置与核心卫星', 'M09纪律监督哨兵', 'KDI监控规模IR风格', '期望值思维工程化落地', '霍华德马克斯橡树思想硬性小节', '魔鬼代言人15问', '预验尸与清零测试', '数据质量ABC分级', '标准深度版默认输出', '执行摘要可选', '禁止动作清单', '认知提升段落', '诚实边界无虚构知识库', '合规与风险提示', 'RAG与网络检索基金数据', '实时数据收集', '概率结论表达', 'CN分析框架与本土化', '行业与政府报告', '数据API与交易所披露', '冰山模型与多源信息'],
            modelPreference: ['deepseek-reasoner', 'glm-4-plus', 'gpt-4o'],
            skills: ['skill_value_investment', 'skill_decision_expert', 'skill_advanced_analytics', 'skill_data_cleaning', 'skill_first_principles', 'skill_analyst', 'skill_researcher', 'skill_swot', 'skill_pyramid', 'skill_mece', 'skill_mermaid_visualization', 'skill_cognitive_psychology', 'skill_iceberg_model', 'skill_planner', 'skill_smart', 'skill_dependency', 'skill_temporal_relation', 'skill_scenario_dynamic_probability', 'skill_multi_temporal_sandbox', 'skill_backtest', 'skill_cn_quality_value', 'skill_cn_multifactor', 'skill_cn_cycle_timing', 'skill_cn_core_satellite', 'skill_cn_smallcap_specialized', 'skill_cn_allweather', 'skill_decision_premortem', 'skill_devil_advocate', 'skill_behavior_guardrails'],
            rules: ['rule_format', 'rule_accuracy', 'rule_examples', 'rule_structure', 'rule_context', 'rule_workflow'],
            mcp: ['mcp_web_search', 'mcp_calculator'],
