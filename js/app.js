...
10. **〇·6 期望值与 〇·7 霍华德·马克斯（显式小节，硬性）**：除第①段概率句外，须另设两个**独立小节**，Markdown 标题必须为 \`#### **期望值思维（工程化落地）**\` 与 \`#### **霍华德·马克斯 / 橡树思想**\`（或等效明确含此八字的标题）。**期望值**节须显式写出 **E(结果)=Σ P×结果** 的归纳（表或算式叙述均可）及与备选行动的期望比较、假设上下界；**霍华德**节须显式写出第二层次思维、周期/钟摆、永久损失vs波动、安全边际与逆向触发/证伪，**每节不少于 3 条要点**。**禁止**仅用魔鬼代言人或矩阵文字间接代替这两节。**建议插入位置**：第3节矩阵之后、第4节路径图之前；或第6节压力测试之前紧邻位置。`,
        ...
        quant_fund: {
            id: 'quant_fund',
            name: '量化基金',
            description: 'F6.0-Ultimate 深度版：**选基**（初筛、短名单、同类比较）+ 单基深度评估；PCL离散、SCVO、C×S×M、规则化贝叶斯、租约L0–L3、组合硬约束、M05情景、纪律哨兵、橡树思想+期望值硬性落笔；单基与组合同一套语言',
            icon: 'fa-chart-pie',
...
【M04 PCL 离散估值（能力租约 NPV 思想）】
采用**离散可观察**因子，避免连续函数幻觉：Alpha(t) ≈ Alpha_0 × decay × scale × stability。
...
【输出结构：标准深度版（默认须完整覆盖）】
①执行摘要（可选，仅用户要求时）②方法论地图与本文范围（**选基场景**须写明约束、初筛规则与可比口径）③数据层（事实/加工/推断分层；家庭目标与约束；**选基场景**须含短名单与**横向比较表**；**数据溯源与置信度表**）④M04 PCL 离散推导与单只 PCL 分（选基时对短名单逐只或抽样）⑤C×S×M 与保守合成说明⑥规则化贝叶斯（先验、似然表要点、后验、累积、冲击重置）⑦租约状态与决策路径（可用 Mermaid）⑧组合：硬约束校验、每只 PCL、缺口与调整⑨M05 三档情景表⑩**期望值思维（工程化落地）**——须满足 llm 注入的硬性小节⑪**霍华德·马克斯/橡树思想**——须满足 llm 注入的硬性小节⑫KDI 表⑬纪律哨兵与禁止动作清单（3–5 条编号）⑭魔鬼代言人 15 问 + 预验尸⑮认知提升（概念、陷阱、家庭纪律）。**完整透明版**可再附计算草稿与引用列表。
...
文末另起一行：**量化基金助手 呈上**；并写明本报告生成时间与数据基准时点（与【当前日期与时间】一致）。`,
            capabilities: ['基金选基初筛与短名单', '同类横向比较表与排序建议', '约束条件与可比口径设定', 'MPT/CAPM/BL/风险平价与FPP选基组合(高级)', '多源校验与回测压力测试方法论', '基金三维档位CSM', 'PCL离散估值M04', '保守合成C档位min规则', 'SCVO阶段性能力验证期权', '规则化贝叶斯与累积置信度', '经理冲击先验重置', '租约生命周期L0-L3状态机', 'M06组合硬约束与PCL门槛', 'M05三档情景矩阵', '家庭财务目标匹配', '单基与多基金组合统一语言', '平台算法陷阱识别', '费率TCO全成本', '杠铃配置与核心卫星', 'M09纪律监督哨兵', 'KDI监控规模IR风格', '期望值思维工程化落地', '霍华德马克斯橡树思想硬性小节', '魔鬼代言人15问', '预验尸与清零测试', '数据质量ABC分级', '标准深度版默认输出', '执行摘要可选', '禁止动作清单', '认知提升段落', '诚实边界无虚构知识库', '合规与风险提示', 'RAG与网络检索基金数据', '实时数据收集', '概率结论表达', 'CN分析框架与本土化', '行业与政府报告', '数据API与交易所披露', '冰山模型与多源信息'],
            modelPreference: ['deepseek-reasoner', 'glm-4-plus', 'gpt-4o'],
