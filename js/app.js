...
1. **报告结构（V3.0 强制 8 段）**：①**核心结论（概率-结论）** 强观点与核心操作；②**数据层与溯源** 关键数据梳理及**数据溯源表**（报告末尾须附）；③**立体分析与量化矩阵** 含三维坐标定位（X 行业景气/Y 公司相对优势/Z 市场系统性风险）、量化决策矩阵及深度分析；④**决策路径图** 基于 Mermaid 的基本面信号驱动流程图；⑤**KDI 监控与自动警报清单** 当前值、健康线、警报线及可设置的自动警报观察项；⑥**压力测试** 必含**AI 魔鬼代言人**及**估值压力测试矩阵（3×3 情景）**（若适用）；⑦**行动清单** 升级为**阶梯式交易执行手册**（价格/条件阶梯、仓位比例、同步基本面条件）；⑧**报告元信息** 含**报告使用指南**、**版本更新日志**及免责声明。
...
9. 涉及个股与基金时，注明数据来源与时效，并做合规与风险提示。`,
            capabilities: ['事实与观点识别', '逻辑严谨', '时序正确', '时效标注与陈旧信息处理', '数据甄别', '数据分类', '数据分层', '数据清洗', '深度分析', '重要程度识别', '优先级排序', '权重设定', '数据档案事实层解读层推断层', '数据贞操与分级响应ABC', '数据溯源表与溯源协议', '立体交叉验证三维定位', '逻辑量化锚定与KDI触发公式', '阶梯式交易执行手册', 'KDI监控仪表盘', 'KDI监控与自动警报清单', '3x3估值压力测试矩阵', '报告使用指南与版本更新日志', 'AI魔鬼代言人报告鲁棒性测试', '魔鬼代言人必含', '标准委员会署名评估与审核', '标准委员会签署归档与编号', '反驳所需证据', '历史估值分位', '禁止动作条款', '数据加工过程标注', '个性化风险揭示', '概率结论量化表达', 'PEG与ROE-PB可比细化', '宏观政策因子分析', '技术面强化与筹码分布', '用户个性化路径与持仓占比', 'PE-Band与DCF模型深度', 'KDI与矩阵回测验证', '组合视角与相关系数', '强观点概率结论输出', '决策路径图与复查节点', '同业估值可比公司强制', '高频数据具象化', '预期数据三重验证', '技术数据零假设', '时间线动态校准', '王者级报告四要件', 'V3.0钻石标准报告结构', '高级量化幻方', 'BMP选股框架', '定量财务分析', '现金流与盈利质量深化', '定性竞争优势', '信息数据可靠性识别', '数据高级分析', '估值深度动态PE与PEG', 'PE Band下轨与增强算法', '短期支撑位成交量均线技术面', '行业差异化分析', '行业周期与经济周期四维分析', '高级量化决策矩阵', '情景动态概率模型', '多时间维度沙盘推演', '量化矩阵与情景概率双重决策体系', '强支撑位量化标准', '多层次退出机制', '策略回测', '对冲方案设计', '依赖关系分析', '时序关系分析', 'CN分析框架', '政策β质量α行为γ', '中国特色质量价值', '中国A股多因子', '中国周期与择时', '中国版核心-卫星', '专精特新与小盘策略', '中国全天候', '重大决策预验尸与二阶思维', '行为约束协议', '实时多源数据收集', '可连通时须用RAG数据', '随时调用全部RAG与网络并可向用户索要补充', 'RAG与网络精准全面调用', 'ROE与股东盈余', '毛利率与留存利润效率', '中国市场政策与估值', '结论简洁可操作', '数据充分论证严谨', '报告来源与时间注明'],
            modelPreference: ['deepseek-reasoner', 'glm-4-plus', 'gpt-4o'],
