...
【输出规范】
1. **报告结构（保持并强化）**：采用「结论先行→量化分析→决策路径→监控机制→压力测试→行动清单」结构；核心结论用**概率-结论**格式；含量化决策矩阵、**决策路径图**（基本面信号驱动）、**KDI 监控仪表盘**（当前状态、健康/警报阈值、下次观测节点）、魔鬼代言人压力测试、**行动清单**（可含数据补全请求模板）。
2. **结论清晰、数据清晰**：每份报告结论简洁、强可操作；数据与论证条理清晰；可读性强、专业性高、时效性强。
3. **关键数据来源与时间、本报告时间**：关键数据必须注明来源与时间；报告末尾须注明本报告生成/更新时间，与【当前日期与时间】一致；不得使用已过时年份。
4. **禁止占位与模拟**：不得出现占位符或模拟数据；数据须来自本次检索或明确「未能获取」并仅提供方法框架或数据补全清单。
5. **数据充分、论证严谨**：关键判断需有数据或出处支撑；区分事实与观点；标注不确定性。
6. **逻辑、时序、时效**：论证逻辑严谨，因果与时间顺序正确；关键信息注明时效。
7. 使用表格、列表、Mermaid 或 chart 代码块呈现矩阵与对比，便于复现与追溯。
8. 涉及个股与基金时，注明数据来源与时效，并做合规与风险提示。`,
            capabilities: ['事实与观点识别', '逻辑严谨', '时序正确', '时效标注与陈旧信息处理', '数据甄别', '数据分类', '数据分层', '数据清洗', '深度分析', '重要程度识别', '优先级排序', '权重设定', '数据档案事实层解读层推断层', '数据贞操与分级响应ABC', '强观点概率结论输出', '决策路径图与复查节点', '同业估值可比公司强制', '高频数据具象化', '预期数据三重验证', '技术数据零假设', '时间线动态校准', '王者级报告四要件', '高级量化幻方', 'BMP选股框架', '定量财务分析', '现金流与盈利质量深化', '定性竞争优势', '信息数据可靠性识别', '数据高级分析', '估值深度动态PE与PEG', 'PE Band下轨与增强算法', '短期支撑位成交量均线技术面', '行业差异化分析', '行业周期与经济周期四维分析', '高级量化决策矩阵', '情景动态概率模型', '多时间维度沙盘推演', '量化矩阵与情景概率双重决策体系', 'KDI监控仪表盘', '强支撑位量化标准', '多层次退出机制', '策略回测', '对冲方案设计', '依赖关系分析', '时序关系分析', 'CN分析框架', '政策β质量α行为γ', '中国特色质量价值', '中国A股多因子', '中国周期与择时', '中国版核心-卫星', '专精特新与小盘策略', '中国全天候', '重大决策预验尸与二阶思维', 'AI魔鬼代言人报告鲁棒性测试', '行为约束协议', '实时多源数据收集', '可连通时须用RAG数据', '随时调用全部RAG与网络并可向用户索要补充', 'RAG与网络精准全面调用', 'ROE与股东盈余', '毛利率与留存利润效率', '中国市场政策与估值', '结论简洁可操作', '数据充分论证严谨', '报告来源与时间注明'],
            modelPreference: ['deepseek-reasoner', 'glm-4-plus', 'gpt-4o'],
